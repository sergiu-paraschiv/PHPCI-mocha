var mochaPlugin = ActiveBuild.UiPlugin.extend({
    id: 'build-mocha-errors',
    ...
    register: function() {
        var self = this;
        var query = ActiveBuild.registerQuery('mocha-data', -1, {key: 'mocha-data'})

        $(window).on('mocha-data', function(data) {
            self.onUpdate(data);
        });
        ...
        $(document).on('click', '#mocha-data .test-toggle', function(ev) {
            var input = $(ev.target);
            $('#mocha-data tbody ' + input.data('target')).toggle(input.prop('checked'));
        });
    },

    render: function() {

        return $('<div class="table-responsive"><table class="table" id="mocha-data">' +
            '<thead>' +
            '<tr>' +
    ...
    onUpdate: function(e) {
        ...
        var thead = $('#mocha-data thead tr');
        var tbody = $('#mocha-data tbody');
...
ActiveBuild.registerPlugin(new mochaPlugin());
